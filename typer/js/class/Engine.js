define(['Reg','ArticleType','Article', 'Config', 'Record', 'Database', 'KeyCount', 'Utility', 'Editor'], function (
   ...
   let imePreedit = '';             // 组字中的拼音串，Wayland 兜底计数的基准
   let imeKeydownReliable = false;  // 组字期能否收到带 code 的 keydown（Windows / X11 可以，Wayland 不行）
   ...
   class Engine {
      constructor() {
         ...
         typingPad.onkeydown = e => {
            if (e.isComposing && e.code) {
               imeKeydownReliable = true;
            }
         ...
         typingPad.onkeyup = e => {
            e.preventDefault();
            if (!this.isFinished && this.isStarted) {
               // 部分 Linux IME 下 keyup 的 e.key 不可靠，击键统计挪到 keydown
               this.compare();
               // 末字时结束的时候
         ...
         // Wayland + fcitx5/rime 等平台上，组字期间浏览器根本不派发按键事件（实测 keydown/keyup 为 0），
         // 只能靠组字串的增量还原击键数。Windows / X11 组字期能收到带 code 的 keydown，就不走这里。
         typingPad.addEventListener('compositionstart', () => {
            imePreedit = '';
         });
         typingPad.addEventListener('compositionupdate', e => {
            let preedit = e.data || '';
            if (!imeKeydownReliable && preedit.length > imePreedit.length) {
               if (!this.isFinished && !this.isStarted) {
                  this.start();
               }
               if (!this.isFinished && this.isStarted) {
                  keyCount.plusKeys(preedit.length - imePreedit.length);
               }
            }
            imePreedit = preedit;
         });
         typingPad.addEventListener('compositionend', e => {
            // 上屏那一下（空格/数字选词）在 Wayland 下被 IM 吃掉，页面收不到 keydown，按空格补上
            if (!imeKeydownReliable && e.data && !this.isFinished && this.isStarted) {
               keyCount.plusCommitKey();
            }
            // 会话结束就复位：万一某次带 code 的事件根本没组成会话，不至于把兜底永久关掉
            imePreedit = '';
            imeKeydownReliable = false;
         });
      }
